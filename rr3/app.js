document.addEventListener("DOMContentLoaded", () => {
  ...
  if (slider && handle) {
    ...
    const startAutoAnimation = () => {
      console.log("Slider auto-animation starting...");
      ...
      const animate = () => {
        if (!autoAnimating) {
          console.log("Animation canceled or not autoAnimating");
          return;
        }
        const elapsed = performance.now() - startTime;
        
        if (elapsed >= duration) {
          console.log("Animation finished, setting pos to 50%");
          slider.style.setProperty("--slider-pos", "50%");
          autoAnimating = false;
          return;
        }

        // Animate from 50% -> 35% -> 65% -> 50%
        // Using: f(t) = 50 + A * sin(pi * t / D) * sin(2 * pi * t / D)
        // Peak is ~0.77. For 15% amplitude (35% to 65%), A = 15 / 0.77 = 19.5
        const t = elapsed / duration;
        const offset = 19.5 * Math.sin(Math.PI * t) * Math.sin(2 * Math.PI * t);
        const pos = 50 + offset;
    ...
    // Delay start of animation by 1000ms
    setTimeout(() => {
      console.log("Timeout triggered. isDragging:", isDragging, "autoAnimating:", autoAnimating);
